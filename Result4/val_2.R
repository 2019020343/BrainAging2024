...
setwd("D:\\aging\\RESULT\\5val\\snrna\\GENECOUNT")
a<-list.files()
...
write.table(expmean_gene_1,"D:\\aging\\RESULT\\5val\\snrna\\expmean_gene.txt",col.names = T, row.names = T,sep = "\t" ,append = FALSE, quote = F)
rownames(expmean_gene_1)<-expmean_gene_1[,1]
expmean_gene_1<-expmean_gene_1[,-1]


sample_group<-read.table("D:\\aging\\RESULT\\5val\\snrna\\group.txt",header=T,sep = "\t", quote = "")
...
setwd("D:\\aging\\RESULT\\5val\\snrna\\")
...
write.table(extract_ggplotdata,"D:\\aging\\RESULT\\5val\\less40_slope.txt",col.names = T, row.names = F,sep = "\t" ,append = FALSE, quote = F)
cols<-colorRampPalette(c("#5D779B", "#FAF3BE", "#C9301D"))(200)
extract_ggplotdata_less40_test<-read.table("D:\\aging\\RESULT\\5val\\less40_slope.txt",header=T,sep = "\t", quote = "")
