...
library(tidyverse)
#################################Fig.5-A-C  #########################
sample_group <-read.table("D:\\aging\\RESULT\\5val\\DFC\\group.txt",header=T,sep = "\t", quote = "",fill = T)

expmean_gene<-read.table("D:\\aging\\RESULT\\5val\\DFC\\gene_all.txt",header=T,sep = "\t", quote = "")
...
write.table(expmean_gene_1,"D:\\aging\\RESULT\\5val\\DFC\\expmean_gene.txt",col.names = T, row.names = T,sep = "\t" ,append = FALSE, quote = F)
...
###############xcell ###############
library(xCell)
expmean_gene<-read.table("D:\\aging\\RESULT\\5val\\DFC\\expmean_gene.txt",header=T,row.names = 2, sep = "\t", quote = "")
expmean_gene<-expmean_gene[,-1]
sigScores_group<-read.table("D:\\aging\\RESULT\\5val\\DFC\\sigScores_group.txt",header=T,sep = "\t", quote = "")
...
for (i in 1:67) {
  ...
  ggsave(paste("D:\\aging\\RESULT\\5val\\DFC\\xcell\\",celltype[i],".pdf",sep=""), width = 20, height = 20, units = "cm" )
